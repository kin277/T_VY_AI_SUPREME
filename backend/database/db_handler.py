...
def dict_factory(cursor, row):
  """Hàm chuyển đổi kết quả SQLite Row thành Dictionary chuẩn của Python."""
...
def get_memory(user_id: str, key: str) -> Optional[str]:
  ...
  with get_db() as conn:
    ...
    return result.get("memory_value") if result else None
...
def get_all_memories(user_id: str) -> Dict[str, str]:
  ...
  with get_db() as conn:
    ...
    return {row.get("memory_key"): row.get("memory_value") for row in results}
...
def get_db():
  ...
  conn.row_factory = dict_factory  # ✅ Đã sửa thành dict_factory
  try:
    yield conn
  finally:
    conn.close()
...
def create_user(
  ...
  with get_db() as conn:
    conn.execute(
        "INSERT INTO users (id, username, email, password, role, provider,"
        " provider_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def update_subscription(user_id, tier, expiry):
  with get_db() as conn:
    conn.execute(
        "UPDATE users SET subscription_tier = ?, subscription_expiry = ? WHERE"
        " id = ?",
        (tier, expiry, user_id),
    )
    conn.commit()
...
def get_conversation_by_id(conv_id, user_id):
  ...
  with get_db() as conn:
    ...
    if row:
      ...
      try:
        data["messages"] = (
            json.loads(data["messages"]) if data.get("messages") else []
        )
      except:
        data["messages"] = []
      return data
...
def log_usage(user_id, tier):
  ...
  with get_db() as conn:
    conn.execute(
        "INSERT INTO usage_logs (user_id, tier, date, count) VALUES (?, ?, ?,"
        " 1) ON CONFLICT(user_id, tier, date) DO UPDATE SET count = count + 1",
        (user_id, tier, today),
    )
    conn.commit()
...
def get_usage_count(user_id, tier):
  ...
  with get_db() as conn:
    ...
    return log.get("count", 0) if log else 0
...
def get_all_usage_stats():
  ...
  with get_db() as conn:
    result = conn.execute(
        "SELECT SUM(count) as total FROM usage_logs"
    ).fetchone()
...
def get_user_usage_stats(user_id):
  ...
  with get_db() as conn:
    ...
    return {
        "total_usage": total_usage.get("total")
        if total_usage and total_usage.get("total") is not None
        else 0,
        "conversation_count": conv_count.get("count", 0) if conv_count else 0,
    }
